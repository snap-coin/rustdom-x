...
extern crate rustdom_x;

use self::blake2b_simd::blake2b;
use rustdom_x::byte_string::{string_to_u8_array, u8_array_to_string};
use rustdom_x::common::randomx_reciprocal;
use rustdom_x::hash::gen_program_aes_4rx4;
use rustdom_x::m128::m128d;
use rustdom_x::memory::VmMemory;
use rustdom_x::program::{
    Instr, Mode, Opcode, Program, REG_NEEDS_DISPLACEMENT, REG_NEEDS_DISPLACEMENT_IX, Store, a_reg,
    e_reg, f_reg, r_reg,
};
use rustdom_x::vm::{Vm, hash_to_m128i_array, new_register, new_vm};
use std::sync::Arc;
...
fn test_calculate_hash_3_with_full_memory() {
    ...
    let seed = string_to_u8_array(
        "0e0e8bb48b8406bf43039198b7712a35031e0607036ebf9afb3096977e7b8fb88c751430e96b02000006ad82bd221c5e282d0533c5dcca38f30babc2e62cd3aa03a965f8aec8ad6f129f5211",
    );
    let result = vm.calculate_hash(&seed);
    assert_eq!(
